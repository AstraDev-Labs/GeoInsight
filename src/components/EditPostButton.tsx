...
import type { BlogPost, EditPostResponse } from '@/lib/types';
export default function EditPostButton({ post }: { post: BlogPost }) {
    ...
    const handleSave = async () => {
        if (!email || !password) {
            setError('Please enter your author email and secret key for modification.');
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                setError(data.error || 'Intelligence update failed.');
            }
        } catch {
            setError('Network error. Intelligence link aborted.');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <>
            ...
            >
                <Edit3 size={16} className="text-primary" /> Refine Mission Data
            </button>
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-8 mb-12">
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40">Mission Designation (Title)</label>
                                <input
                                    type="text"
                                    value={title}
                            ...
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40">Mission Logs & Analysis</label>
                                <RichTextEditor
                                    content={content}
                                    onChange={setContent}
                        ...
                        <div className="bg-muted/30 border p-8 rounded-2xl mb-12 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-3 mb-6 relative z-10">
                                ...
                                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground">Encryption Protocols</h3>
                            </div>
