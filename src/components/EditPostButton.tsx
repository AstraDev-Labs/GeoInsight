...
export default function EditPostButton({ post }: { post: any }) {
    ...
    return (
        <>
            ...
            >
                <Edit3 size={16} className="text-primary" /> Edit Transmission
            </button>

            {showModal && (
                <div
                    className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[2000] flex items-center justify-center p-4 lg:p-8"
                    onClick={() => setShowModal(false)}
                >
                    <motion.div
                        initial={{ opacity: 0, scale: 0.95, y: 20 }}
                        animate={{ opacity: 1, scale: 1, y: 0 }}
                        className="bg-card border p-8 lg:p-12 w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl relative rounded-[2rem] font-sans"
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-8 mb-12">
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40">Mission Designation</label>
                                <input
                                    type="text"
                                    value={title}
                                    onChange={e => setTitle(e.target.value)}
                                    className="w-full px-6 py-4 bg-muted/30 border text-foreground outline-none focus:border-primary/50 rounded-xl font-bold text-sm"
                                />
                            </div>
                        ...
                        <div className="bg-muted/30 border p-8 rounded-2xl mb-12 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-3 mb-6 relative z-10">
                                ...
                                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground">Security Verification</h3>
                            </div>
