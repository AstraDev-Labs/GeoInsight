...
export default function Navbar() {
    return (
        <nav className="w-full bg-[#0a0a0a] border-b border-white/5 shadow-sm">
            ...
            {/* Mobile nav: horizontally scrollable links */}
            <div className="md:hidden overflow-x-auto border-t border-white/5 scrollbar-hide">
                <div className="flex items-center gap-1 px-3 py-2 min-w-max">
                    <MobileNavLink href="/" icon={<Home size={14} />} label="Home" />
                    <MobileNavLink href="/categories" icon={<Tag size={14} />} label="Categories" />
                    <MobileNavLink href="/request-post" icon={<PlusCircle size={14} />} label="Submit" />
                    <MobileNavLink href="/admin" icon={<LayoutDashboard size={14} />} label="Admin" />
                </div>
            </div>
        </nav>
    );
...
function MobileNavLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
    return (
        <Link
            href={href}
            className="flex items-center gap-1.5 px-3.5 py-2 text-[10px] font-bold uppercase tracking-widest text-white/60 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap"
        >
            {icon} {label}
        </Link>
    );
