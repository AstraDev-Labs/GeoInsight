...
export default function RequestPost() {
    ...
    const uploadFiles = async (files: File[]): Promise<string[]> => {
        ...
        const uploadPromises = files.map(async (file) => {
            try {
                // 1. Try to generate a presigned upload URL from the server
                ...
                if (presignRes.ok) {
                    ...
                    // 2. Upload directly from the browser to Cloudflare R2
                    ...
                    if (uploadRes.ok) {
                        return presignData.publicUrl; // Success
                    } else {
                        console.warn('Direct R2 upload failed, falling back to local server upload.');
                    }
                }
            } catch (err) {
                console.warn('Presign direct upload failed or unsupported, trying fallback upload:', err);
            }

            // Fallback: local dev file upload using /api/upload multipart
